...
import { getAllUsers, makeUserAdmin, makeUserVendor, markUserAsFraud } from "@/lib/actions/admin";
...
useEffect(() => {
  ...
  // Event Handlers
    ...
    const handleConfirmFraud = async () => {
      ...
      try {
        ...
        toast.error(error.message || "Error marking user as fraudulent");
      }
    };
  ...
  return (
    <div className="min-h-screen text-slate-800 dark:text-slate-200 font-sans p-4 sm:p-6 md:p-10 antialiased selection:bg-indigo-500 selection:text-white transition-colors duration-200">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Main Card Container */}
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-xl dark:shadow-2xl p-4 sm:p-6 md:p-8 transition-colors">
          {/* Header Section */}
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800/80">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <select
                ...
                className="bg-slate-100 dark:bg-[#1e2736] text-slate-800 dark:text-slate-200 text-sm rounded-xl px-4 py-2 border border-slate-300 dark:border-slate-700/60 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer"
              >
                ...
                {filteredUsers.length > 0 ? (
                  filteredUsers.map((user) => {
                    ...
                    return (
                      <tr
                        key={userId}
                        className="bg-slate-50 dark:bg-[#1a2332]/60 hover:bg-slate-100 dark:hover:bg-[#1f2a3c] transition-colors rounded-xl border border-slate-200 dark:border-slate-800/40"
                      >
                        ...
                        <td className="px-4 py-3.5 text-slate-600 dark:text-slate-400 font-normal">
                          {user.email}
                        </td>
                          ...
                          <div className="inline-flex items-center justify-end gap-2">
                            <button
                              ...
                              className={`px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                                isAdmin
                                  ? "bg-indigo-100 dark:bg-indigo-600/40 text-indigo-400 dark:text-indigo-300/50 cursor-not-allowed"
                            ...
                            <button
                              ...
                              className={`px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                                isVendor
                                  ? "bg-sky-100 dark:bg-sky-600/40 text-sky-400 dark:text-sky-300/50 cursor-not-allowed"
                            ...
                            {userRole === "vendor" && (
                              <button
                                ...
                                className={`px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                                  user.isFraud
                                    ? "bg-red-100 dark:bg-red-600/30 text-red-400 dark:text-red-300/40 border border-red-200 dark:border-red-500/20 cursor-not-allowed"
            ...
            {filteredUsers.length > 0 ? (
              filteredUsers.map((user) => {
                ...
                return (
                  <div
                    key={userId}
                    className="bg-slate-50 dark:bg-[#1a2332] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-md dark:shadow-lg"
                  >
                    <div className="flex items-center gap-4">
                      <img
                        src={userAvatar}
                        alt={user.name}
                        className="w-14 h-14 rounded-full object-cover border-2 border-slate-300 dark:border-slate-700 shadow-md"
                      />
                      <div>
                        ...
                        <div className="mt-1">
                          {user.isFraud ? (
                            <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-red-500/10 dark:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/30 uppercase">
                              Fraud Vendor
                            </span>
                          ) : (
                      ...
                      <div className="flex flex-col gap-2">
                        <button
                          ...
                          className={`w-full py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${
                            isAdmin
                              ? "bg-indigo-100 dark:bg-indigo-600/30 text-indigo-400 dark:text-indigo-300/40 cursor-not-allowed"
                        ...
                        <button
                          ...
                          className={`w-full py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${
                            isVendor
                              ? "bg-sky-100 dark:bg-sky-600/30 text-sky-400 dark:text-sky-300/40 cursor-not-allowed"
                        ...
                        {userRole === "vendor" && (
                          <button
                            ...
                            className={`w-full py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${
                              user.isFraud
                                ? "bg-red-100 dark:bg-red-600/20 text-red-400 dark:text-red-400/40 border border-red-200 dark:border-red-500/20 cursor-not-allowed"
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 mt-6 border-t border-slate-200 dark:border-slate-800/80 text-xs text-slate-500 dark:text-slate-400 font-medium">
            ...
            <div className="flex items-center gap-2">
              <button className="px-2 py-1 rounded bg-slate-200 dark:bg-[#1e2736] hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 disabled:opacity-30">
                &lt;
              </button>
              <button className="px-2.5 py-1 rounded bg-indigo-600 text-white font-bold">
                1
              </button>
              <button className="px-2.5 py-1 rounded bg-slate-200 dark:bg-[#1e2736] hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300">
                2
              </button>
              <button className="px-2.5 py-1 rounded bg-slate-200 dark:bg-[#1e2736] hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300">
                3
              </button>
              <button className="px-2 py-1 rounded bg-slate-200 dark:bg-[#1e2736] hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300">
                &gt;
              </button>
            </div>
          </div>
      ...
      {/* Fraud Warning Modal */}
      {selectedFraudVendor && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-[#151c28] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
              ...
              <strong className="text-slate-900 dark:text-white">
                {selectedFraudVendor.name}
              </strong>{" "}
              as a fraudulent vendor?
            </p>

            <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl p-3.5 text-xs text-red-700 dark:text-red-300 space-y-1">
            ...
            <div className="flex justify-end gap-3 pt-3">
              <button
                ...
                className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-[#1e2736] rounded-xl border border-slate-300 dark:border-slate-700 transition"
              >
                Cancel
              </button>
              <button
