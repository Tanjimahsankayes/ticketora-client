...
const DashboardLayout = ({ children }) => {
  ...
  if (isPending) {
    return (
      <div className="min-h-screen w-full flex flex-col items-center justify-center bg-slate-950 text-slate-200">
        ...
        <p className="mt-4 text-xs font-medium tracking-widest uppercase text-slate-400 animate-pulse">
          Loading Dashboard...
        </p>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-slate-100 antialiased selection:bg-indigo-500 selection:text-white flex flex-col md:flex-row overflow-x-hidden">
      {/* Background Ambient Glows */}
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
        <div className="absolute -top-[10%] -left-[10%] w-[45%] h-[45%] rounded-full bg-indigo-500/5 blur-[120px]" />
        <div className="absolute top-[60%] -right-[10%] w-[40%] h-[40%] rounded-full bg-blue-500/5 blur-[120px]" />
      </div>

      {/* Sidebar Section */}
      <aside className="relative z-20 w-full md:w-72 md:shrink-0 md:sticky md:top-0 md:h-screen border-b md:border-b-0 md:border-r border-slate-800/80 bg-slate-900/60 backdrop-blur-xl transition-all duration-300">
        {renderSidebar()}
      </aside>
